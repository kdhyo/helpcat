...
class MessageUserDetail extends Component {
  render() {
    ...
    if(opponentData.UserOnRoom[0].user[0].id === myData.id || opponentData.UserOnRoom[1].user[0].id === myData.id) {
      const roomId = opponentData.UserOnRoom[0].roomId
      return (
        <>
        <Link to={`/message/${roomId}`}>
          <div className="chatting-box">
            <li className="chatting-user">{opponentData.UserOnRoom[0].user[0] ===  myData.id? opponentData.UserOnRoom[1].user[0].nickName : opponentData.UserOnRoom[0].user[0].nickName}</li><br></br>
